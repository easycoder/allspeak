!   chat-main.as - Chat client with login, topics, and posts
...
!    debug step
    ...
    attach ShowTopicsButton to `show-topics-button`
    ...
    attach BackToTopicsButton to `back-to-topics`
    attach PostsTopicTitle to `posts-topic-title`
    ...
    attach BackToPostsButton to `back-to-posts`
    attach ReplyToggle to `reply-toggle`
    attach PostViewSubject to `post-view-subject`
    attach PostViewMeta to `post-view-meta`
...
!    log Credentials
    ...
    rest get SystemID from `chat-id`
...
EnterChat:
    ...
    on click ShowTopicsButton gosub LoadTopics
    on click NewTopicToggle gosub ToggleNewTopic
    on click CreateTopicButton gosub CreateTopic
    on click CancelTopicButton gosub HideNewTopic
    on click BackToTopicsButton gosub BackToTopics
    ...
    on click BackToPostsButton gosub BackToPosts
...
ToggleNewTopic:
    set style `display` of NewTopicPanel to `flex`
    return
...
ShowPosts:
    set the content of PostsTopicTitle to CurrentTopic
...
ViewPost:
    ...
    set the content of PostViewMeta to `by ` cat PAuthor
...
RenderReplies:
    ...
    begin
        ...
        put RepliesMarkup
            ...
            cat `<strong>` cat RAuthor cat `:</strong> ` cat RPreview
            cat `</div>` into RepliesMarkup
        add 1 to N
    end
...
BackToPosts:
    ...
    set style `display` of PostsTopicTitle to `block`
    set style `display` of NewPostToggle to `inline-block`
    set style `display` of BackToTopicsButton to `inline-block`
    gosub HideReply
    return
