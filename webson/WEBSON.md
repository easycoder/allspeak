...
Webson is a JSON-based UI rendering system used by AllSpeak.
...
- `"@id": "room-/ROOM/"` (often pre-replaced in AllSpeak before render)
...
- `render` resumes AllSpeak only after Webson build completes
...
- Use `@id` consistently for AllSpeak `attach` targets
- Prefer declarative `#switch`/`#repeat` over manual string assembly
- Keep top-level script focused on layout; load dynamic data in AllSpeak and then render

## Typical workflow in this project

1. AllSpeak fetches Webson JSON (`rest get ...`)
2. AllSpeak runs `render Webson in Parent`
3. AllSpeak attaches DOM variables by known IDs
4. AllSpeak binds click handlers / updates content / style as state changes
...
- IDs such as `system-title`, `system-name`, `mainpanel`, `profile-button`, `statusflag` are defined declaratively and then attached in AllSpeak.
...
AllSpeak later toggles visibility by style updates, avoiding imperative DOM creation code.
...
AllSpeak substitutes `/ROOM/` before render, allowing one JSON template to generate all room rows consistently.
