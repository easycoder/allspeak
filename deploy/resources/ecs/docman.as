! Docman
  ...
  append `mqttpy.json` to Content
  append `psutil.json` to Content
  append `rest.json` to Content
  append `server.json` to Content
...
Viewer:
  ...
  put property `versions` of Record into Versions
  put `Version(s): ` into Value
  if Versions includes `js`
    put Value cat `&#9745; JS` into Value
  else
    put Value cat `&#9744; JS` into Value
  if Versions includes `python`
    put Value cat ` &nbsp; &#9745; Python` into Value
  else
    put Value cat ` &nbsp; &#9744; Python` into Value
  create RowDiv in ContentDiv
  set the style of RowDiv to `margin:0.5em 0;color:#555;font-size:0.9em`
  set the content of RowDiv to Value
...
! Decorator callback — m:, l:, q: are now handled by the showdown fallbackTransform
Decorate:
  stop
...
NewPackage:
  set the text of Status to `New package is disabled in static hosting mode`
  wait 3
...
SavePackage:
  gosub to SaveKeyword
  set the text of Status to `Save is disabled in static hosting mode`
  wait 3
  set the text of Status to ``
  stop