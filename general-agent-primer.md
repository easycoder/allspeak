...
Use this 2-step flow in AllSpeak scripts:
...
For non-AllSpeak stacks, use the equivalent step/trace debugger pattern and keep the same investigation mindset.

## 6A) AllSpeak Loop Syntax Guard

When generating AllSpeak loops:
...
## 6C) AllSpeak Event-Handler Guard

When generating AllSpeak interaction logic for repeated elements:
...
## 6D) AllSpeak Array Access Guard

When generating AllSpeak array logic:
...
## 6E) AllSpeak Formatting Convention

When generating AllSpeak scripts:
...
## 8A) AllSpeak Capability Rule

When implementing AllSpeak features:
