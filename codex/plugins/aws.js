const AllSpeak_AWS = {
	...
	AWS: {
		...
		run: (program) => {
			...
			if (command.request === `setUrl`) {
				AllSpeak_AWS.url = program.getValue(command.url);
				return command.pc + 1;
			} else if ([`get`, `post`, `delete`].includes(command.request)) {
				const method = command.request.toUpperCase();
				const url = `${AllSpeak_AWS.url}${program.getValue(command.url)}`;
				...
				case `post`:
					...
					console.log(`POST to ${AllSpeak_AWS.url}`);
	...
	run: (program) => {
		const command = program[program.pc];
		const handler = AllSpeak_AWS.getHandler(command.keyword);
		if (!handler) {
...
AllSpeak.domain.aws = AllSpeak_AWS;
