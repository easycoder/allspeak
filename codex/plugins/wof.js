	...
	*/
	...
	Draw: {
		...
		run: program => {
			...
			const wheel = AllSpeak_roulette_wheel;
	...
	getHandler: (name) => {
		switch (name) {
		case `draw`:
			return AllSpeak_WOF.Draw;
		case `spin`:
			return AllSpeak_WOF.Spin;
		case `wheel`:
			return AllSpeak_WOF.Wheel;
		default:
			return null;
		}
	},
	...
	run: (program) => {
		const command = program[program.pc];
		const handler = AllSpeak_WOF.getHandler(command.keyword);
		if (!handler) {
...
// eslint-disable-next-line no-unused-vars
AllSpeak.domain.wof = AllSpeak_WOF;
