const AllSpeak_SVG = {
	...
	getHandler: (name) => {
		...
		case `create`:
			return AllSpeak_SVG.Create;
		case `ellipse`:
			return AllSpeak_SVG.Ellipse;
		case `group`:
		...
		case `svg`:
			return AllSpeak_SVG.SVG;
		case `svgtext`:
			return AllSpeak_SVG.SVGText;
		default:
			return null;
		}
	},
	...
	run: (program) => {
		const command = program[program.pc];
		const handler = AllSpeak_SVG.getHandler(command.keyword);
		if (!handler) {
...
AllSpeak.domain.svg = AllSpeak_SVG;
