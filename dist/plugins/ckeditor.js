const AllSpeak_CKEditor = {

	name: `AllSpeak_CKEditor`,
	...
	getHandler: (name) => {
		switch (name) {
		case `ckeditor`:
			return AllSpeak_CKEditor.CKEditor;
		default:
			return null;
		}
	},
	...
	run: (program) => {
		const command = program[program.pc];
		const handler = AllSpeak_CKEditor.getHandler(command.keyword);
		if (!handler) {
...
AllSpeak.domain.ckeditor = AllSpeak_CKEditor;
