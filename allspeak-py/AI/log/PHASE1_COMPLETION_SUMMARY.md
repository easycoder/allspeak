...
- **Implementation**: Value parsing in `as_value.py` recognizes both forms
- **Plugin-safe**: Articles and prepositions act as delimiters; plugins can define bare `cat` verb without collision

**Code Changes**:
- `as_value.py`: Added `the cat of` pattern recognition in compileValue()
- `as_core.py`: Documentation updated showing canonical and shorthand forms
...
**Code Changes**:
- `as_core.py`: k_fork, k_go, k_gosub updated with `skip('to')`
...
**Code Changes**:
- `as_compiler.py`: Added skipArticles() method
- `as_handler.py`: Exposed skipArticles() to all handlers (core, graphics, plugins)
...
- `as_handler.py`: Added skipArticles method reference to all handlers
...
### Modified Files
```
allspeak/as_compiler.py                       - Added skipArticles() method
allspeak/as_handler.py                        - Exposed skipArticles to handlers
allspeak/as_graphics.py                       - Updated k_set, k_remove with skipArticles + comments
allspeak/as_value.py                          - Added "the cat of" pattern recognition
allspeak/as_core.py                           - Updated fork/go/gosub to use skip('to')
...
- allspeak-py/.github/copilot-instructions.md — Original project guidance
