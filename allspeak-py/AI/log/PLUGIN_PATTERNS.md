...
This guide provides plugin developers with **proven patterns** for extending AllSpeak safely, avoiding collision with core and graphics reserved stems while maintaining readability and consistency with AllSpeak's English-like syntax philosophy.
...
AllSpeak uses articles ("the"), prepositions ("to", "of", "in", "from"), and conjunctions ("and") as **disambiguation delimiters**. Leverage these:
...
Follow AllSpeak convention (variables are CapitalCase, keywords are lowercase):
...
```python
# plugins/json.py
from allspeak.as_handler import Handler
import json
...
- [ ] Examples follow AllSpeak conventions (CapitalCase variables, lowercase keywords)
...
3. **Search codebase**: `grep -r "your_keyword" allspeak/`
...
| ✅ Follow AllSpeak naming (lowercase keywords) | ❌ Use CamelCase for keywords |
...
To contribute a plugin to AllSpeak:
