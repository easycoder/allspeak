# AllSpeak Syntax Refactoring & Standardization Plan
...
AllSpeak's strength is its extensibility via plugins. The core module must:
...
1. **Add `set … to …` support** in `as_core.py` (mirror existing `put … into …` logic).
...
From `as_compiler.py` and `as_handler.py`:
