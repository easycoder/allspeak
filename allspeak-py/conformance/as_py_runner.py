...
Usage:
    python3 conformance/as_py_runner.py \
...
def run_script(script_path: Path) -> dict[str, Any]:
    ...
    buf = io.StringIO()
    try:
        from allspeak import Main as ECMain  # local import to keep module-level clean
    except ImportError:
        return {"logs": [], "error": {"category": "setup", "message": "allspeak package not installed"}}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="AllSpeak Python-CLI conformance adapter")
