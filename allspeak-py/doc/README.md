...
***AllSpeak*** is a combined compiler and runtime for a high-level English-like script. Compilation is very quick - typically of the order of 10 lines per millisecond - so there is no need to run the compiler as a separate pass.
...
***AllSpeak*** has several ways to handle values that have more than one element. There are 3 basic variable types:
...
At the current stage of development there is usually just one way to express a given function. English is replete with alternatives that all mean the same thing and additional words that carry no actual meaning but make a sentence easier to read, and there is no reason why many of these cannot be implemented in ***AllSpeak*** as a means to make scripts more readable. An example might be
...
'Packages' are components of the ***AllSpeak*** system that deal with specific groups of language features. Those included or available from the repository are
