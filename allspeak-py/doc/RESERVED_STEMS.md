...
This document lists keywords and word stems reserved by the **core** AllSpeak module. Plugin developers should avoid claiming these as standalone verbs to prevent collision with existing or future core syntax.

## Purpose

AllSpeak's extensibility depends on clear boundaries between core and plugin namespaces. **Syntactic noise** (articles like "the", prepositions like "to"/"of") serves as disambiguation, but plugins must also respect reserved stems to ensure future-proof interoperability.
...
- **Monitor updates**: Check this document with each AllSpeak release.
